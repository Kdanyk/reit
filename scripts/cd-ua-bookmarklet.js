(() => {
  if (window.scanCounterCDUA_V28) return;
  window.scanCounterCDUA_V28 = true;
  document.querySelectorAll('[data-reit-counter]').forEach((el) => el.remove());

  const saveKey = 'scanCounterCDUA_V28State';
  ...
  let triggerText = 'Елемент завершено', nlpText = 'Відскануйте новий NLP';
  ...
  function initCounts() { hours.forEach((h) => { if (hourCounts[h] == null) hourCounts[h] = 0; }); }
  ...
  function loadState() {
    try {
      ...
      manualColor = s.manualColor || '#a0a0a0';
      miniPos = s.miniPos || 'bl';
      miniOpacity = Math.min(100, Math.max(0, s.miniOpacity !== undefined ? parseInt(s.miniOpacity) : 100));
      miniSize = Math.min(45, Math.max(10, parseInt(s.miniSize) || 18));
      hourCounts = {};
      hours.forEach((h) => { hourCounts[h] = Math.max(0, parseInt(s.hourCounts && s.hourCounts[h]) || 0); });
  ...
  function saveState(force) {
    ...
    lastSave = now;
    try {
      localStorage.setItem(saveKey, JSON.stringify({ shift: shiftName, savedAt: now, start, beforeBreak, targetPerHour, showLeftInsteadTotal, autoStatusColor, manualColor, miniPos, miniOpacity, miniSize, hourCounts, lastTrigger }));
    } catch (_) {}
  }
  ...
  <div id="mainView" style="width:100%; box-sizing:border-box;">
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; border-bottom:1px solid rgba(0,0,0,0.06); padding-bottom:10px;">
      <div style="font-size:18px; font-weight:900; color:#0f172a; letter-spacing:0.5px; text-transform:uppercase;">Clean-Decant UA</div>
      <button id="settingsBtn" title="Налаштування" style="width:30px; height:30px; border:none; border-radius:8px; background:rgba(0,0,0,0.05); color:#0f172a; font-size:16px; cursor:pointer; transition:background 0.2s;">⚙️</button>
    </div>
    <div style="display:grid; grid-template-columns:1fr; gap:8px; margin-bottom:10px; width:100%; box-sizing:border-box;">
      <div style="background:#ffffff; border:1px solid rgba(0,0,0,0.06); border-radius:10px; padding:10px;">
        <div style="font-size:10px; color:#64748b; text-transform:uppercase; margin-bottom:4px; font-weight:700;">Тригер дії</div>
        <div id="lt" style="font-size:12px; font-weight:700; color:#1e293b; word-break:break-all; line-height:1.2;">-</div>
      </div>
    </div>
    <div style="display:grid; grid-template-columns:1fr; gap:8px; margin-bottom:12px; width:100%; box-sizing:border-box;">
      <div style="background:linear-gradient(135deg, #3b82f6, #2563eb); color:white; padding:12px 8px; border-radius:10px; text-align:center; text-transform:uppercase;">
        <div style="font-size:10px; font-weight:900; opacity:0.9; margin-bottom:2px;">Залишилося</div>
  ...
  <div id="settingsView" style="display:none; width:100%; box-sizing:border-box;">
    <div style="display:flex; align-items:center; margin-bottom:12px; border-bottom:1px solid rgba(0,0,0,0.06); padding-bottom:10px;">
      <button id="backBtn" title="Назад" style="width:30px; height:30px; border:none; border-radius:8px; background:rgba(0,0,0,0.05); color:#0f172a; font-size:18px; cursor:pointer; margin-right:10px; display:flex; align-items:center; justify-content:center;">‹</button>
      <div style="font-size:16px; font-weight:900; color:#0f172a; text-transform:uppercase; letter-spacing:0.5px;">Налаштування</div>
    </div>
    <div style="background:#ffffff; border:1px solid rgba(0,0,0,0.06); border-radius:10px; padding:12px; margin-bottom:10px; width:100%; box-sizing:border-box;">
      <div style="display:grid; grid-template-columns:105px 1fr; gap:8px; align-items:center; font-size:11px; font-weight:700; color:#475569; text-transform:uppercase; width:100%; box-sizing:border-box;">
        ...
        <select id="pos" style="width:100%; height:28px; border:1px solid #e2e8f0; border-radius:6px; cursor:pointer; background:#fff; font-family:\${technoFont}; font-weight:700; box-sizing:border-box; margin:0;">
          <option value="bl" \${miniPos === 'bl' ? 'selected' : ''}>Низ - Ліво</option>
          <option value="br" \${miniPos === 'br' ? 'selected' : ''}>Низ - Право</option>
          <option value="tl" \${miniPos === 'tl' ? 'selected' : ''}>Верх - Ліво</option>
          <option value="tr" \${miniPos === 'tr' ? 'selected' : ''}>Верх - Право</option>
        </select>
    ...
    <div style="background:#ffffff; border:1px solid rgba(0,0,0,0.06); border-radius:10px; padding:12px; margin-bottom:10px; width:100%; box-sizing:border-box;">
      <label style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; font-size:11px; font-weight:900; text-transform:uppercase; color:#1e293b; cursor:pointer;">Залишилось замість суми <input id="leftMode" type="checkbox" style="width:18px; height:18px; accent-color:#3b82f6; margin:0; cursor:pointer;" \${showLeftInsteadTotal ? 'checked' : ''}></label>
      <label style="display:flex; justify-content:space-between; align-items:center; font-size:11px; font-weight:900; text-transform:uppercase; color:#1e293b; cursor:pointer;">Авто-колір темпу <input id="autoColor" type="checkbox" style="width:18px; height:18px; accent-color:#3b82f6; margin:0; cursor:pointer;" \${autoStatusColor ? 'checked' : ''}></label>
    </div>
    <div style="background:#ffffff; border:1px solid rgba(0,0,0,0.06); border-radius:10px; padding:12px; margin-bottom:16px; width:100%; box-sizing:border-box;">
      <div style="font-size:10px; font-weight:900; text-transform:uppercase; margin-bottom:8px; color:#64748b;">Попередній перегляд</div>
  ...
  while (settingsView.children.length > 1) settingsHost.appendChild(settingsView.children[1]); settingsView.appendChild(tableBox);
  ...
  function addPacks(n) {
    ...
    hourCounts[getSlot()] += n; recalcTotal(); lastTrigger = 'РУЧНО +' + n + ' ' + timeNow(); saveState(true); render();
  }
  
  function removePack() {
    const slot = getSlot();
    if (hourlyTotal() > 0) { hourCounts[slot] = Math.max(0, hourCounts[slot] - 1); recalcTotal(); lastTrigger = 'РУЧНО -1 ' + timeNow(); saveState(true); render(); }
  }
  ...
  function bindCountInputs() {
    panel.querySelectorAll('.hc').forEach((inp) => {
      ...
      inp.onblur = (e) => { e.target.value = hourCounts[e.target.getAttribute('data-h')] || 0; lastTrigger = 'РУЧНО ' + timeNow(); renderHours(true); saveState(true); render(); };
    });
    const bb = panel.querySelector('#beforeBreak');
    if (bb) {
      bb.oninput = (e) => { beforeBreak = Math.max(0, parseInt(e.target.value) || 0); recalcTotal(); applyMini(); panel.querySelector('#left').textContent = Math.max(0, shiftTarget() - total); saveState(); };
      bb.onblur = (e) => { e.target.value = beforeBreak || 0; lastTrigger = 'РУЧНО ' + timeNow(); renderHours(true); saveState(true); render(); };
    }
  }
  ...
  function renderHours(force) {
    ...
    rows += \`<div style="display:grid; grid-template-columns:85px 45px 1fr; gap:10px; align-items:center; background:#f8fafc; border:1px solid rgba(0,0,0,0.06); border-radius:8px; padding:6px 10px; margin-top:12px; margin-bottom:6px; border-left:4px solid #94a3b8; width:100%; box-sizing:border-box;">
      <b style="font-size:11px; text-align:left; color:#475569;">До перерви</b>
  ...
  function scan() {
    ...
    if (nlpm > nlpp) { 
      skipNextPack = true; 
      lastTrigger = 'NLP: ПРОПУСТИ НАСТУПНУ ' + timeNow(); 
      render(); 
    }
    
    if (m > p) { 
      let diff = m - p; 
      if (skipNextPack) { diff--; skipNextPack = false; lastTrigger = 'ПРОПУЩЕНО ПО NLP ' + timeNow(); } 
